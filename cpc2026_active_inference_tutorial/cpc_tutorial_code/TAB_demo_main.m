...
sim_block_probs = [.8 .4 .5];
if SIM
    [sim_mdp, gen_data] = TAB_sim(ALPHA, CR, ETA_WIN, ETA_LOSS, PRIOR_A,OMEGA,sim_block_probs);
...
if FIT
    if SIM
        fit_results = TAB_sim_fit(gen_data,sim_block_probs);
...
%and the Fs for M2 are greater (indicating higher log model evidence and a better fit)
Fs_M2 = Fs_M1 + 2;
