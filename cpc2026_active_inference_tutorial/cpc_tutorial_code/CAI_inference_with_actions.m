...
t = 6;              % [s] Simulation time. Inference occurs over 6 seconds.
h = 0.001;          % [s] Integration step. The arm moves every 1/1000 seconds.
actionsTime = 1;    % [s] Actions (i.e., adjustments to the arm) occur after one second.
% Planar 2DOF robot parameters
a1 = 1;             % [m] Length link 1. This is the length of the first section of the arm (i.e., bicep)
a2 = 1;             % [m] Length link 2. This is the length of the second section of the arm (i.e., forearm)
...
% Initialize the vector of beliefs about the states and its derivatives
mu = zeros(2,t/h);     
mu_p = zeros(2,t/h);
mu_pp = zeros(2,t/h);   
...
for i=1:t/h-1
    ...
    % State estimation
    mu(:,i+1) = mu(:,i) + h*mu_dot;             % Belief about the position; new belief is old belief plus prediction error (i.e., change in position)
    mu_p(:,i+1) = mu_p(:,i) + h*mu_dot_p;       % Belief about motion of mu; new belief is old belief plus prediction error (i.e., change in velocity)
    mu_pp(:,i+1) = mu_pp(:,i) + h*mu_dot_pp;    % Belief about motion of mu'; new belief is old belief plus prediction error (i.e., change in acceleration)
