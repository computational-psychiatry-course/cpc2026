...
y_max = max([a_con_1(:); a_con_2(:); a_con_3(:)]) + 1;

% --- Win concentration parameter (row 1 of each a_con) ---
subplot(2,2,3)

h1 = plot(a_con_1(1,:),'.','MarkerSize',12,'Color',col{1}); hold on
plot(a_con_1(1,:),':','Color',col{1})

h2 = plot(a_con_2(1,:),'.','MarkerSize',12,'Color',col{2});
plot(a_con_2(1,:),':','Color',col{2})

h3 = plot(a_con_3(1,:),'.','MarkerSize',12,'Color',col{3});
plot(a_con_3(1,:),':','Color',col{3})

title('Win Concentration Parameter')
xlim([0,n_trials+2])
ylim([0,y_max])
xlabel('Trial'),ylabel('Value')
set(gca, 'XTick', [1:n_trials+1])
set(gca, 'XTickLabel', [0:n_trials])
legend([h1 h2 h3], {'Bandit 1','Bandit 2','Bandit 3'}, 'Location','best')
hold off

% --- Loss concentration parameter (row 2 of each a_con) ---
subplot(2,2,4)

h1 = plot(a_con_1(2,:),'.','MarkerSize',12,'Color',col{4}); hold on
plot(a_con_1(2,:),':','Color',col{4})

h2 = plot(a_con_2(2,:),'.','MarkerSize',12,'Color',col{5});
plot(a_con_2(2,:),':','Color',col{5})

h3 = plot(a_con_3(2,:),'.','MarkerSize',12,'Color',col{6});
plot(a_con_3(2,:),':','Color',col{6})

title('Loss Concentration Parameter')
xlim([0,n_trials+2])
ylim([0,y_max])
xlabel('Trial'),ylabel('Value')
set(gca, 'XTick', [1:n_trials+1])
